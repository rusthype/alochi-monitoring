...
class ResultScreen extends StatefulWidget {
  ...
  const ResultScreen({super.key, required this.session, required this.package,
      required this.result, required this.synced,
      this.xpEarned = 0, this.wrongAnswers = const []});
...
class _ResultScreenState extends State<ResultScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _celebCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 3400))
      ..forward();

    // Overlay fade out
    _overlayFadeCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 400));
    ...
    _cardFade  = CurvedAnimation(parent: _cardCtrl, curve: const Interval(.3, 1, curve: Curves.easeOut));
    _cardSlide = Tween<Offset>(begin: const Offset(0, .04), end: Offset.zero)
        .animate(CurvedAnimation(parent: _cardCtrl, curve: const Interval(.3, 1, curve: Curves.easeOut)));
    ...
    return Container(
      ...
      child: Stack(children: [
        ...
        Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              child: Transform.translate(
                ...
                child: ShaderMask(
                  shaderCallback: (bounds) => const LinearGradient(
                    colors: [Color(0xFFFBBF24), Color(0xFFF97316), Color(0xFFFB923C)],
                  ).createShader(bounds),
          ...
          Text(
            _passed ? "Test muvaffaqiyatli yakunlandi" : "Keyingi safarida yaxshiroq bo'ladi!",
            ...
            TweenAnimationBuilder<double>(
              ...
              builder: (_, v, __) => Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: Color.lerp(Colors.white.withOpacity(.2), AppColors.brand, v),
                ),
              ),
            ),
          )),
            ...
            child: SlideTransition(
              ...
              child: Column(children: [
                ...
                AnimatedBuilder(
                  ...
                  builder: (_, __) => CustomPaint(
                    ...
                    painter: _RingPainter(
                      ...
                      color: mainColor,
                      bgColor: mainColor.withOpacity(.1),
                    ),
                    child: SizedBox(width: 150, height: 150,
                      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                        Text('$_pct%', style: TextStyle(
                          fontSize: 36, fontWeight: FontWeight.w800,
                          color: mainColor, fontFamily: 'JetBrainsMono',
                        )),
                        Text('ball', style: TextStyle(
                          fontSize: 11, color: mainColor.withOpacity(.7),
                          fontWeight: FontWeight.w700, letterSpacing: .09,
                        )),
                      ]),
                    ),
                  ),
                ...
                Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Icon(_passed ? Icons.check_circle_rounded : Icons.cancel_rounded,
                        size: 16, color: Colors.white),
                    const SizedBox(width: 7),
                    Text(_passed ? "Tabriklaymiz! O'tdingiz!" : "O'tmadi. Harakat qiling!",
                        style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Colors.white)),
                  ]),
                ),
                const SizedBox(height: 8),
                Text(widget.session.studentName,
                    style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: AppColors.ink1)),
                const SizedBox(height: 4),
                Text('${widget.session.grade}-sinf · Variant ${widget.session.variant}',
                    style: const TextStyle(fontSize: 12, color: AppColors.ink3,
                        fontFamily: 'JetBrainsMono')),
                const SizedBox(height: 24),
                // Subject cards
                Row(children: [
                  Expanded(child: _SubjectCard(
                    label: 'Matematika', icon: Icons.calculate_outlined,
                    score: widget.result.mathScore,
                    total: widget.package.mathCount, color: const Color(0xFF3B82F6),
                    ring: _ring,
                  )),
                  const SizedBox(width: 12),
                  Expanded(child: _SubjectCard(
                    label: 'Ingliz tili', icon: Icons.language_rounded,
                    score: widget.result.engScore,
                    total: widget.package.engCount, color: const Color(0xFF0D9488),
                    ring: _ring,
                  )),
                ]),
                const SizedBox(height: 12),
                ...
                Container(
                  ...
                  child: Row(children: [
                    const Icon(Icons.equalizer_rounded, size: 18, color: AppColors.ink2),
                    const SizedBox(width: 8),
                    const Text('Jami ball', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.ink2)),
                    const Spacer(),
                    Text('${widget.result.mathScore + widget.result.engScore} / ${widget.package.totalCount}',
                        style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800,
                            color: AppColors.ink1, fontFamily: 'JetBrainsMono')),
                  ]),
                ),
                const SizedBox(height: 12),
                // Sync
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: (widget.synced ? AppColors.ok : AppColors.ink3).withOpacity(.08),
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(color: (widget.synced ? AppColors.ok : AppColors.border).withOpacity(.4)),
                  ),
                  child: Row(children: [
                    Icon(widget.synced ? Icons.cloud_done_outlined : Icons.cloud_off_outlined,
                        size: 15, color: widget.synced ? AppColors.ok : AppColors.ink3),
                    const SizedBox(width: 8),
                    Expanded(child: Text(
                      widget.synced
...
class _SubjectCard extends StatelessWidget {
  ...
  const _SubjectCard({required this.label, required this.icon, required this.score,
      required this.total, required this.color, required this.ring});
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.ink3)),
      ]),
      const SizedBox(height: 8),
      Text('$score', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800,
          color: AppColors.ink1, fontFamily: 'JetBrainsMono')),
      Text('/ $total', style: const TextStyle(fontSize: 11, color: AppColors.ink3,
          fontFamily: 'JetBrainsMono')),
      const SizedBox(height: 8),
      ClipRRect(
...
class _Particle {
  ...
  static const _colors = [
    Color(0xFFF97316), Color(0xFFFBBF24), Color(0xFF4ADE80),
    Color(0xFF60A5FA), Color(0xFFA78BFA), Color(0xFFF472B6),
    Color(0xFFFDE68A), Color(0xFF34D399),
  ];
...
class _ConfettiPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final p in particles) {
      ...
      final alpha   = (p.opacity * (py < 0.8 ? 1.0 : (1.1 - py) / 0.3)).clamp(0.0, 1.0);
...
class _WrongAnswersSection extends StatefulWidget {
  ...
  @override State<_WrongAnswersSection> createState() => _WrongAnswersSectionState();
}
...
class _WrongAnswersSectionState extends State<_WrongAnswersSection> {
  ...
  @override
  Widget build(BuildContext context) {
    final math = widget.wrongAnswers.where((w) => w.subject == 'math').length;
    final eng  = widget.wrongAnswers.where((w) => w.subject == 'english').length;

    return Container(
      decoration: BoxDecoration(
        color: AppColors.surface,
        borderRadius: BorderRadius.circular(14),
        border: Border.all(color: AppColors.err.withOpacity(.25)),
      ),
      child: Column(children: [
        // Header
        InkWell(
          ...
          child: Padding(
            ...
            child: Row(children: [
              Container(
                width: 32, height: 32,
                decoration: BoxDecoration(
                  color: AppColors.err.withOpacity(.1),
                  borderRadius: BorderRadius.circular(8),
                ),
                child: Icon(Icons.close_rounded, color: AppColors.err, size: 18),
              ),
              const SizedBox(width: 12),
        ...
        if (_expanded) ...[
          ...
          ...widget.wrongAnswers.asMap().entries.map((e) {
            final w    = e.value;
            final sAns = _answerLabel[w.studentAnswer] ?? w.studentAnswer.toUpperCase();
            final cAns = _answerLabel[w.correctAnswer] ?? w.correctAnswer.toUpperCase();
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Container(
                    ...
                    child: Text('${w.position}', style: const TextStyle(
                        fontSize: 11, fontWeight: FontWeight.w700,
                        color: AppColors.ink2, fontFamily: 'JetBrainsMono')),
                  ),
                  const SizedBox(width: 8),
                  Text(_subjectLabel[w.subject] ?? w.subject,
                      style: const TextStyle(fontSize: 11, color: AppColors.ink3,
                ...
                if (w.image != null) ...[
                  ...
                  ClipRRect(
                    borderRadius: BorderRadius.circular(8),
                    child: Image.network(w.image!, height: 80, fit: BoxFit.contain,
                        errorBuilder: (_, __, ___) => const SizedBox.shrink()),
                  ),
                ],
                const SizedBox(height: 8),
                _AnswerChip(label: sAns, text: w.studentText, isCorrect: false),
                const SizedBox(height: 4),
                _AnswerChip(label: cAns, text: w.correctText, isCorrect: true),
              ]),
            );
          }),
        ],
...
class _AnswerChip extends StatelessWidget {
  ...
  const _AnswerChip({required this.label, required this.text, required this.isCorrect});
  ...
  Widget build(BuildContext context) {
    ...
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(child: Text(text, style: TextStyle(
          fontSize: 12, color: isCorrect ? AppColors.ok : AppColors.err,
