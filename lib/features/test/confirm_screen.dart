...
class ConfirmScreen extends StatefulWidget {
  ...
  const ConfirmScreen({super.key, required this.session, required this.package});
  @override
...
class _ConfirmScreenState extends State<ConfirmScreen> {
  ...
  Future<void> _start() async {
    ...
    try {
      final questions = await api.getQuestions(widget.package.id, widget.session.variant);
      if (!mounted) return;
      Navigator.pushReplacement(context, MaterialPageRoute(
        builder: (_) => TestScreen(session: widget.session, package: widget.package, questions: questions)));
    } catch (e) {
      setState(() { _error = "Yuklanmadi: $e"; _loading = false; });
    }
  }
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  CircleAvatar(
                    ...
                    child: Text(s.studentName.isNotEmpty ? s.studentName[0] : 'T',
                        style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: AppColors.brand)),
                  ),
                  const SizedBox(height: 16),
                  const Text("Salom!", style: TextStyle(color: AppColors.ink2)),
                  const SizedBox(height: 4),
                  Text(s.studentName, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w800)),
                  const SizedBox(height: 12),
                  Wrap(spacing: 8, runSpacing: 8, alignment: WrapAlignment.center, children: [
                    _chip('${s.grade}-sinf', Icons.school_outlined),
                    _chip('Variant ${s.variant}', Icons.numbers_outlined),
                    if (s.groupName != null) _chip(s.groupName!, Icons.group_outlined),
                  ]),
                  ...
                  Container(
                    ...
                    child: Column(children: [
                      ...
                      Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
                        _stat('${p.mathCount}', 'Matematika', AppColors.math),
                        Container(width: 1, height: 36, color: AppColors.border),
                        _stat('${p.engCount}', 'Ingliz tili', AppColors.eng),
                        Container(width: 1, height: 36, color: AppColors.border),
                  ...
                  if (_error != null) ...[
                    const SizedBox(height: 12),
                    Text(_error!, style: const TextStyle(color: AppColors.err, fontSize: 13), textAlign: TextAlign.center),
                  ],
                  const SizedBox(height: 24),
                  ElevatedButton.icon(
                  ...
                  TextButton(
                    ...
                    child: const Text('Orqaga', style: TextStyle(color: AppColors.ink2)),
                  ),
                ]),
              ),
            ),
