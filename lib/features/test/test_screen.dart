...
class _TestScreenState extends State<TestScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Vaqt: har savol uchun 1.2 daqiqa (minimum 30, max 60 daqiqa)
    final totalQ = widget.questions.length;
    final mins   = (totalQ * 1.2).ceil().clamp(30, 60);
    _timerSecs   = mins * 60;
    _startTimer();
  ...
  Future<void> _finish() async {
    ...
    if (unanswered > 0) {
      final ok = await showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
          ...
          title: const Text('Tugatish?', style: TextStyle(fontWeight: FontWeight.w800)),
          content: Text('$unanswered ta savol javobsiz qoldi. Shunga qaramay tugatmoqchimisiz?'),
          actions: [
            TextButton(onPressed: () { _startTimer(); Navigator.pop(context, false); }, child: const Text('Orqaga')),
            ElevatedButton(
              onPressed: () => Navigator.pop(context, true),
              style: ElevatedButton.styleFrom(backgroundColor: AppColors.brand, minimumSize: const Size(100, 40)),
    ...
    final result = TestResult(
      packageId: widget.package.id, variant: widget.session.variant,
      mathScore: mathAns, engScore: engAns,
    ...
    // Parse to typed WrongAnswer list
    final rawWrong    = resp['wrong_answers'] as List<dynamic>? ?? [];
    final wrongAnswers = rawWrong
        .whereType<Map<String, dynamic>>()
        .map((j) => WrongAnswer.fromJson(j))
        .toList();
    if (!synced) await OfflineQueue.enqueue(result);
    if (!mounted) return;
    Navigator.pushReplacement(context, MaterialPageRoute(
        builder: (_) => ResultScreen(
            session: widget.session, package: widget.package,
            result: result, synced: synced, xpEarned: xpEarned,
          ...
          child: Column(children: [
            ...
            SizedBox(
              ...
              child: Stack(children: [
                Container(width: double.infinity, height: 4,
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    Text(widget.session.studentName,
                        style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.ink1)),
                    const SizedBox(width: 8),
                    _GradePill(grade: widget.session.grade),
                  ]),
                  const SizedBox(height: 2),
                  Text(_isMath ? 'Matematika bo\'limi' : 'Ingliz tili bo\'limi',
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                  Expanded(child: SingleChildScrollView(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      // Section banner
                      if (_showSectionBanner) _SectionBanner(
                          onDismiss: () => setState(() => _showSectionBanner = false)),
                      ...
                      AnimatedSwitcher(
                        ...
                        transitionBuilder: (child, anim) => FadeTransition(
                          ...
                          child: SlideTransition(
                            position: Tween<Offset>(begin: const Offset(.015, 0), end: Offset.zero).animate(anim),
                            child: child,
                          ),
                        ),
                        child: Column(
                        ...
                        child: Stack(children: [
                          ...
                          Positioned(left: 0, top: 0, bottom: 0,
                            child: AnimatedContainer(
                              ...
                              width: 4,
                              decoration: BoxDecoration(
                                color: _isMath ? AppColors.math : AppColors.eng,
                              ),
                            ),
                          ),
                          // Content padding
                              ...
                              Row(children: [
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(color: AppColors.bg, borderRadius: BorderRadius.circular(20),
                                      border: Border.all(color: AppColors.border)),
                                  child: Text(_isMath ? 'Matematika' : 'Ingliz tili',
                                ...
                                Row(mainAxisSize: MainAxisSize.min, children: [
                                  for (final k in ['A', 'B', 'C', 'D']) ...[
                                    _KbdKey(label: k),
                                    if (k != 'D') const SizedBox(width: 3),
                                  ],
                                  const SizedBox(width: 5),
                                  const Text('tugmalar', style: TextStyle(fontSize: 10, color: AppColors.ink3)),
                                ]),
                              ]),
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      AnimatedOpacity(
                        ...
                        child: SizedBox(
                          ...
                          child: OutlinedButton.icon(
                            onPressed: _isFirst ? null : () => _navigate(_current - 1),
                            icon: const Icon(Icons.arrow_back_rounded, size: 15),
                            label: const Text('Oldingi', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
                      ...
                      Expanded(child: Column(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        const Text('javoblandi', style: TextStyle(fontSize: 11, color: AppColors.ink3)),
                      ])),
                      SizedBox(
                        width: 110, height: 44,
                        child: _isLast
                          ? ElevatedButton.icon(
                              ...
                              label: Text(_submitting ? 'Yuklash' : 'Tugatish',
                          ...
                          : ElevatedButton.icon(
                              ...
                              icon: const Text('Keyingi', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
...
class _GradePill extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final colors = {
      1: (const Color(0xFFFFF7ED), const Color(0xFF9A3412), const Color(0xFFFED7AA)),
      2: (const Color(0xFFF0FDFA), const Color(0xFF0F766E), const Color(0xFF99F6E4)),
      3: (const Color(0xFFEFF6FF), const Color(0xFF1E40AF), const Color(0xFFBFDBFE)),
      4: (const Color(0xFFF5F3FF), const Color(0xFF6D28D9), const Color(0xFFDDD6FE)),
    };
    final c = colors[grade] ?? colors[1]!;
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
      decoration: BoxDecoration(
        color: c.$1, border: Border.all(color: c.$3), borderRadius: BorderRadius.circular(20),
      ),
...
class _TimerWidget extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => AnimatedContainer(
    ...
    decoration: BoxDecoration(
      ...
      border: Border.all(color: isHot ? const Color(0xFFFCA5A5) : const Color(0xFFFED7AA), width: 1.5),
    ...
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text('qoldi', style: TextStyle(
          fontSize: 9, fontWeight: FontWeight.w700, letterSpacing: .09,
...
class _QDot extends StatelessWidget {
  ...
  const _QDot({required this.number, required this.isCurrent,
      required this.isAnswered, required this.isMath, required this.onTap});

  @override Widget build(BuildContext context) {
    final accent = isCurrent ? AppColors.brand
        : isAnswered ? const Color(0xFF86EFAC)
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: isCurrent
                  ? [BoxShadow(color: AppColors.brand.withOpacity(.3),
...
class _SectionBanner extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      gradient: const LinearGradient(colors: [Color(0xFFFFF7ED), Colors.white]),
    ...
    child: Row(children: [
      ...
      const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text("Ingliz tili bo'limiga o'tildi",
            style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF7C2D12))),
        SizedBox(height: 2),
        Text("40 daqiqa · Keyingi savollar ingliz tilida",
            style: TextStyle(fontSize: 11, color: AppColors.ink3)),
      ])),
      IconButton(onPressed: onDismiss, icon: const Icon(Icons.close_rounded, size: 18, color: AppColors.ink3)),
    ]),
  );
}
...
class _OptionRow extends StatefulWidget {
  ...
  const _OptionRow({required this.label, required this.text,
      this.optionImage, required this.selected, required this.onTap});
  @override State<_OptionRow> createState() => _OptionRowState();
}
...
class _OptionRowState extends State<_OptionRow> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _barCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 720));
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: widget.selected
              ? [BoxShadow(color: AppColors.brand.withOpacity(.12), blurRadius: 10, offset: const Offset(0,3))]
              : [BoxShadow(color: Colors.black.withOpacity(.03), blurRadius: 3, offset: const Offset(0,1))],
        ),
        child: Stack(children: [
          Padding(
            ...
            child: Row(children: [
              ...
              Stack(children: [
                AnimatedContainer(
                  ...
                  width: 34, height: 34,
                  decoration: BoxDecoration(
                    color: widget.selected ? AppColors.brand : const Color(0xFFF4F4F5),
                    border: Border.all(
                        color: widget.selected ? AppColors.brand : const Color(0xFFD4D4D8), width: 1.5),
                    borderRadius: BorderRadius.circular(10),
                  ),
                  child: Center(child: Text(widget.label,
                      style: TextStyle(fontWeight: FontWeight.w800, fontSize: 13,
                          color: widget.selected ? Colors.white : const Color(0xFFA1A1AA)))),
                ),
                // Keyboard hint superscript
                Positioned(top: -1, right: -1, child: Container(
                  width: 13, height: 13,
                  decoration: BoxDecoration(
                    color: widget.selected ? const Color(0xFFFED7AA) : const Color(0xFFD4D4D8),
                    borderRadius: BorderRadius.circular(3),
                    border: Border.all(color: widget.selected
                        ? const Color(0xFFFED7AA) : const Color(0xFFE4E4E7)),
                  ),
                  child: Center(child: Text(widget.label,
                      style: TextStyle(fontSize: 7, fontWeight: FontWeight.w700,
                          color: widget.selected ? const Color(0xFF9A3412) : const Color(0xFFA1A1AA)))),
                )),
              ]),
              const SizedBox(width: 14),
              // Option text
              Expanded(child: widget.optionImage != null
                ? Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    if (widget.text.isNotEmpty) ...[
                      Text(widget.text, style: TextStyle(
                          fontSize: 14, fontWeight: FontWeight.w500,
                          color: widget.selected ? const Color(0xFF7C2D12) : AppColors.ink1)),
                      const SizedBox(height: 6),
                    ],
                    ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.network(
                      widget.optionImage!, height: 80, fit: BoxFit.contain,
                      errorBuilder: (_, __, ___) => const Icon(
                          Icons.broken_image_outlined, color: AppColors.ink3, size: 20),
                    )),
                  ])
                : Text(widget.text, style: TextStyle(
                    fontSize: 14, fontWeight: FontWeight.w500,
...
class _KbdKey extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Container(
    ...
    child: Text(label, style: const TextStyle(
        fontSize: 10, fontWeight: FontWeight.w700, color: AppColors.ink3)),
  );
}
